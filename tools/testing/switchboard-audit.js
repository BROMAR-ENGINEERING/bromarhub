...
window.BromarTest.SwitchboardAudit = (function () {
  ...
  function renderForm(container, config) {
    ...
    container.innerHTML = `
      ...
      <div class="field-row">
        ...
        <div class="field-group"><label>Site Name</label><input type="text" id="saSiteName" value="${esc(cfg.siteName || '')}"></div>
      </div>
      <div class="field-row">
        <div class="field-group"><label>Site Address</label><input type="text" id="saSiteAddress" value="${esc(cfg.siteAddress || '')}"></div>
        ...
        jobInput.addEventListener('input', () => {
          ...
          jobResults.querySelectorAll('.autocomplete-item').forEach(item => {
            item.addEventListener('click', () => {
              ...
              const cEl = container.querySelector('#saClient'), snEl = container.querySelector('#saSiteName'), saEl = container.querySelector('#saSiteAddress');
              if (cEl && !cEl.value) cEl.value = item.dataset.cn;
              if (snEl && !snEl.value) snEl.value = item.dataset.sn;
              if (saEl && !saEl.value) saEl.value = item.dataset.sa;
            });
          });
        });
  ...
  function collectData(container) {
    ...
    return {
      jobNumber: g('saJobNumber').trim() || '', client: g('saClient').trim(),
      siteName: g('saSiteName').trim(), siteAddress: g('saSiteAddress').trim(),
  ...
  async function generatePDF(data, jobNumber, auditRev) {
    ...
    y = pairRow([['Client', data.client], ['Site Name', data.siteName]], y);
    y = pairRow([['Site Address', data.siteAddress], ['Switchboard ID', data.boardId]], y);
    y = pairRow([['Location', data.location], ['Auditor', data.auditor]], y);
    y = pairRow([['Date', data.date], ['Job Number', jobNumber]], y);
    y += 2;
